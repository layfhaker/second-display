...
| Что | Статус |
|-----|--------|
| **Расширение рабочего стола на планшет** (вирт. дисплей, курсор, тач) | ✅ **работает** (30 fps @ 1920×1280, ~1.4 МБ/с) |
| Зеркало основного экрана (HEVC + тач + курсор) | ✅ работает |
| GPU zero-copy (DXGI + VPP + D3D11-вход энкодера) | ✅ работает на обновлённом Intel-драйвере; `--cpu` — запасной путь |
| Стилус с нажатием (OPPO Pen) | ⏳ отложено («фаза 2.5») |

> Хост работает с приоритетом **High**. Под 100% загрузкой CPU (тяжёлые сборки вроде `cargo build`)
> энкодер голодает и стрим проваливается — не запускайте сборки одновременно с работой вторым экраном.
