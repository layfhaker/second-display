...
**Источник найден: сам MFT-энкодер Intel.** Изоляция по звеньям:
...
| полный тракт без клиента (306 кадров) | 59 МБ/с |

Настройка MFT в Rust и в C#-эталоне **идентична** (атрибуты, типы, порядок вызовов, сообщения),
значит это не наша ошибка использования, а утечка в драйверном MFT QuickSync — вероятно, приехала с
недавним **обновлением драйвера Intel** (проблемы с памятью начались примерно тогда). Утечка
**общепроцессная**: пересоздание энкодера её не возвращает (проверено — память росла монотонно при
двух рециклах MFT), значит освободить её может только перезапуск процесса.
