...
(при утечке свободно 9.3 ГБ); темп тот же в режиме `--cpu`, значит **не D3D11/VPP**; самотест
энкодера (120 кадров, «HEVC encoder OK») памяти не жрёт, значит **не MFT сам по себе**. Остаётся
что-то общее для обоих режимов — источник ещё сужается.
...
- **«Вылетающие окна» объяснены и исправлены.** Причина: хост на каждом старте делал
  `vdd.disable()` + `restore_extend()`, а затем `vdd.enable()` — несколько изменений топологии
  дисплеев, а Windows на каждое раскладывает все окна заново. Из-за перезапусков сторожа это
  повторялось каждые ~полторы минуты, и пользователь это видел как «второй дисплей заработал —
  окна вылетели». Теперь **уже поднятый виртуальный дисплей переиспользуется без изменений**
  (`VddController::active_monitors`, `Orchestrator::vdd_active`): на рестарте топология не трогается
  вообще. Проверено в логе: `Virtual display already up — reusing it as is` →
  `Reusing the virtual display \\\\.\\DISPLAY20` → стрим 29.6 fps.
