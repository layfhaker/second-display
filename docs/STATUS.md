...
Разбор `%LOCALAPPDATA%\SecondDisplay\host.log` вскрыл три независимых механизма:

1. **Гейт готовности требовал MTP/PTP.** На ColorOS планшет часто остаётся в режиме «только
   adb», и хост ждал бесконечно (в логе — 55 минут `USB mode is not data transfer`). Теперь
   `GetDeviceReadiness` проверяет только `Awake` + `unlocked`; туннель `adb reverse`
   переприменяется в `WaitForClient` при необходимости.
2. **Реконнект-шторм.** Во время столла/пересоздания энкодера (5–12 с без кадров) клиент ловил
   20-секундный таймаут чтения и переподключался, а каждый реконнект заново пересоздавал
   encoder/decoder → снова чёрный кадр. Хост теперь шлёт `PING` (пустой пакет) каждые 2 с, пока
   нет видео (`Server.cs` SendLoop, `Protocol.Ping`) — клиент переживает пересоздание.
3. **Watchdog энкодера бил по «просто медленному» энкодеру.** Порог был 6 с, а iGPU-столлы
   бывали 4.6–7.8 с и восстанавливались сами — пересоздание (≈5–12 с черноты) только усугубляло.
   Порог поднят до 12 с; пересоздаём только реально faulted/зависший энкодер.

Плюс:
- Клиент: watchdog зависшего декодера (кадры приходят, а рендера нет >4 с) и самовосстановление,
  если поток декодирования тихо умер (раньше замирало навсегда без реконнекта).
- `adb`: `adb devices` с таймаутом 3 с (не 10), shell-проба 6 с, `reverse --remove` 3 с, реже
  рестарт adb-сервера (дебаунс 20 с), `pnputil` 8 с (чтобы не падать в медленный PowerShell).

Ещё не сделано: force keyframe вместо пересоздания энкодера при подключении клиента; убрать
мёртвый UDP/RNDIS-код из `Server.cs`.
