; SecondDisplay C# Host installer (Inno Setup 6).
; Built with scripts\build-installers.ps1.

#define MyAppName "SecondDisplay (C#)"
#define MyAppPublisher "SecondDisplay"
#define HostExeName "SecondDisplay.Host.exe"
#define TaskName "SecondDisplayHostCSharp"
...
#ifndef SourceDir
  #define SourceDir "..\build\staging-csharp"
#endif

; Pass /DLangRu or /DLangEn for single-language installers; neither = multi-language.
#ifdef LangRu
  #define OutName "SecondDisplay-CSharp-Setup-" + MyAppVersion + "-ru"
#else
  #ifdef LangEn
    #define OutName "SecondDisplay-CSharp-Setup-" + MyAppVersion + "-en"
  #else
    #define OutName "SecondDisplay-CSharp-Setup-" + MyAppVersion
  #endif
#endif
...
[CustomMessages]
#ifdef LangRu
AutostartTask=Запускать SecondDisplay (C#) при входе в систему (нужны права администратора)
AutostartGroup=Автозапуск:
RunNow=Запустить SecondDisplay (C#) сейчас
UninstallProgram=Удалить {#MyAppName}
#else
  #ifdef LangEn
AutostartTask=Start SecondDisplay (C#) automatically at logon (requires administrator privileges)
AutostartGroup=Autostart:
RunNow=Start SecondDisplay (C#) now
UninstallProgram=Uninstall {#MyAppName}
  #else
en.AutostartTask=Start SecondDisplay (C#) automatically at logon (requires administrator privileges)
en.AutostartGroup=Autostart:
en.RunNow=Start SecondDisplay (C#) now
en.UninstallProgram=Uninstall {#MyAppName}
ru.AutostartTask=Запускать SecondDisplay (C#) при входе в систему (нужны права администратора)
ru.AutostartGroup=Автозапуск:
ru.RunNow=Запустить SecondDisplay (C#) сейчас
...
[Files]
; Host executable and all dependencies / runtimes
