; SecondDisplay Assembly Host installer (Inno Setup 6).
; Built with scripts\build-installers.ps1.

#define MyAppName "SecondDisplay (Assembly)"
#define MyAppPublisher "SecondDisplay"
#define HostExeName "SecondDisplay.Host.Asm.exe"
#define TaskName "SecondDisplayHostAsm"
...
#ifndef SourceDir
  #define SourceDir "..\build\staging-asm"
#endif

; Pass /DLangRu or /DLangEn for single-language installers; neither = multi-language.
#ifdef LangRu
  #define OutName "SecondDisplay-Asm-Setup-" + MyAppVersion + "-ru"
#else
  #ifdef LangEn
    #define OutName "SecondDisplay-Asm-Setup-" + MyAppVersion + "-en"
  #else
    #define OutName "SecondDisplay-Asm-Setup-" + MyAppVersion
  #endif
#endif

[Setup]
AppId={{C9F3B8E5-2E4A-4F8B-AD32-6E9E3C7B1F24}
AppName={#MyAppName}
...
[CustomMessages]
#ifdef LangRu
AutostartTask=Запускать SecondDisplay (Assembly) при входе в систему (нужны права администратора)
AutostartGroup=Автозапуск:
RunNow=Запустить SecondDisplay (Assembly) сейчас
UninstallProgram=Удалить {#MyAppName}
#else
  #ifdef LangEn
AutostartTask=Start SecondDisplay (Assembly) automatically at logon (requires administrator privileges)
AutostartGroup=Autostart:
RunNow=Start SecondDisplay (Assembly) now
UninstallProgram=Uninstall {#MyAppName}
  #else
en.AutostartTask=Start SecondDisplay (Assembly) automatically at logon (requires administrator privileges)
en.AutostartGroup=Autostart:
en.RunNow=Start SecondDisplay (Assembly) now
en.UninstallProgram=Uninstall {#MyAppName}
ru.AutostartTask=Запускать SecondDisplay (Assembly) при входе в систему (нужны права администратора)
ru.AutostartGroup=Автозапуск:
ru.RunNow=Запустить SecondDisplay (Assembly) сейчас
